...
async def create_missing_views(db: _db):  # noqa: PLR0915
    ...
    # 检查底层数据库类型
    is_sqlite = False
    if hasattr(db, '_original_prisma'):
        # 通过 _active_provider 判断数据库类型
        is_sqlite = db._original_prisma._active_provider == 'sqlite'
    ...
    try:
        ...
        if is_sqlite:
            ...
            "LiteLLM_SpendLogs" 
            WHERE 
            startTime >= strftime('%Y-%m-%d', 'now', '-30 day')
            GROUP BY 
            date(startTime);
            """
        else:
    ...
    try:
        ...
        if is_sqlite:
            ...
            L.api_key = V.token
            WHERE
            L.startTime >= strftime('%Y-%m-%d', 'now', '-30 day')
            GROUP BY
    ...
    try:
        ...
        if is_sqlite:
            ...
            SELECT
                json_extract(request_tags, '$') AS individual_request_tag,
                date(s.startTime) AS spend_date,
                COUNT(*) AS log_count,
                SUM(COALESCE(spend, 0)) AS total_spend
            FROM "LiteLLM_SpendLogs" s
            WHERE request_tags IS NOT NULL AND request_tags != '[]'
            GROUP BY request_tags, date(s.startTime);
            """
        else:
            sql_query = """
            CREATE VIEW DailyTagSpend AS
    ...
    try:
        ...
        if is_sqlite:
            ...
            AND startTime >= strftime('%Y-%m-%d', 'now', '-30 day')
            GROUP BY end_user
...
async def should_create_missing_views(db: _db) -> bool:
    ...
    try:
        ...
        # 获取行数
        row_count = (
            result[0]["count"] if is_sqlite 
            else result[0]["reltuples"]
        )

        verbose_logger.debug(
            "Estimated Row count of LiteLLM_SpendLogs = {}".format(result)
        )

        # 检查是否需要创建视图
        if (row_count is not None and 
            (row_count == 0 or row_count == -1)):
        ...
        verbose_logger.debug(f"Error checking row count: {str(e)}")
        # 如果出错，建议创建视图
        return True
